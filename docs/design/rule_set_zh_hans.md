...
> 集成）已实现；阶段 4 更新示例配置与文档（见 §12）。

## 摘要（Abstract）

本设计为 dae-rs 引入文本域名/IP 列表的规则集支持：

- **数据格式**："每行一条"的纯文本域名/IP 列表。
...
- **引用语法**：`source_ip(set:chinaip)` / `target_domain(set:chinadomain)` 等，
  并保留/映射现有 `dip()`/`sip()`/`domain()` 等语法。
- **自由组合**：完整复刻原版 dae 的规则组合语义（AND/OR/NOT/多条件单 action/
  顺序匹配/fallback/`any`）。
- **eBPF 集成**：IP 数据编译进 LPM 前缀树，域名数据编译进 `domain_sets`，
  明确容量约束与超限时的拒绝/降级策略。
...
| # | 位置 | 缺陷 |
|---|------|------|
| 1 | [`matcher.rs`](control/src/routing/matcher.rs:964) `parse_cidr_values()` | `set:xx` 引用无法解析，CIDR 解析失败被静默丢弃 |
| 2 | [`matcher.rs`](control/src/routing/matcher.rs:1626) `build_match_set_for_function()` | `set:cn` 等解析失败被**静默丢弃**（`if let Ok(cidrs)` 分支不命中则无任何 MatchSet 生成） |
| 3 | [`matcher.rs`](control/src/routing/matcher.rs:1242) 域名收集 | `domain(set:cn)` 被当作普通后缀域名模式 `cn` 处理 → 退化为"以 .cn 结尾"匹配 |
| 4 | 全局 | 无数据文件加载/解析/更新机制，无数据源配置项 |
...
- **数据面匹配**经 [`compile_rules()`](control/src/routing/matcher.rs:1172) 编译进 eBPF：
  ...
  - 域名经 `domain_sets`（每条域名规则一个 `Vec<String>` 模式集）→ eBPF
    `domain_sets` 匹配，由 [`tproxy.c`](bpf/kern/tproxy.c) 查询。
...
- dae 原生支持文本域名/IP 列表与 `geoip:`/`geosite:` 数据文件。
- dae-rs 当前仅支持文本域名/IP 列表（`domain_list` / `ip_list`）。
...
1. **多条件 AND**：一条规则内的多个匹配条件用 `&&` 组合，全部命中才执行动作。
    例如 `dport(443) && domain(set:cn) -> direct`。
2. **条件内 OR**：单个函数内的多个参数用逗号分隔，命中任意一个即算该条件命中。
    例如 `dport(80,443)`。
3. **NOT 取反**：条件前加 `!` 表示取反，如 `!domain(set:cn)`。
...
- 数据文件与元数据分离：`.tmp/`、`.checksum/`、`.meta/` 为**隐藏目录**，仅
  dae-rs 读写。
- **命名空间隔离**：文件名 = `<唯一 name>.txt`。name 由配置指定（§5），用于
  路由引用（`set:<name>`）与文件定位。
...
- 加载后的规则集解析为**内存数据结构**：
  - `ip_list`：`Vec<IpNet>`；
  - `domain_list`：`Vec<DomainPattern>`（保留 type：suffix/regex/full/domain/keyword）。
- 更新完成后，通过现有 eBPF 双缓冲 epoch 机制重新执行
  [`compile_rules()`](control/src/routing/matcher.rs:1172)，原子切换（对齐现有热重载）。
...
| 字段 | 必填 | 类型 | 说明 |
|------|------|------|------|
| `type` | 是 | enum | `domain_list`（文本域名）、`ip_list`（文本 IP） |
...
解析阶段，将 `sip`→`source_ip`、`dip`/`ip`→`target_ip`，并识别 `set:` 前缀，
统一进入 `Function { name, params }` IR。`domain(...)` 继续作为普通域名函数。

### 6.3 数据面（matcher / `compile_rules()`）求值语义

- **`source_ip(set:chinaip)` / `target_ip(set:chinaip)`**：在
  [`parse_cidr_values()`](control/src/routing/matcher.rs:960) 中识别 `set:<name>`
  前缀 → 查内存缓存得到 `Vec<IpNet>` → 经
...
- **`target_domain(set:chinadomain)`**：在 [`compile_rules()`](control/src/routing/matcher.rs:1242)
  域名收集阶段，把 Domain 列表映射为 domain_sets 的模式条目（`suffix:`/`full:`/
  `regex:`/`domain:`/`keyword:`），生成 `MatchType_DomainSet` MatchSet。
- **编译期缺失处理**：若 `set:<name>` 在内存缓存中不存在（数据未下载/损坏）：
  - 静默丢弃不再被允许（修复现状缺陷）；
...
| 文本解析失败 | 视为损坏，按 §4.3 恢复 | `error` | 数据缺失时相关规则编译报错（E2103，§6.3） |
...
- **IP 列表** → LPM 前缀树：
  - 每个 `set:<ip_list>` 引用解析为一组 `IpNet`，经
...
- **域名列表** → `domain_sets` 模式：
  - 每个 `set:<domain_list>` 引用映射为一个 domain_sets 条目（含 suffix/full/
    regex/domain 模式）。
...
| 单个 LPM trie 内部条目 | `MAX_LPM_SIZE = 2_048_000` | 单 trie 可容纳 200 万 CIDR |
| domain_routing_map 位图 | 32 个 u32（=1024 bit） | **域名集规则索引上限 1024**（每个 set 域名引用占 1 bit） |

- **瓶颈在 MatchSet 总数与域名集索引数**（规则数量）。
- **注意**：域名集索引 = 编译期出现的 `target_domain(set:*)` / `domain(...)`
  规则条目数（按引用去重），上限 1024。
...
- **优先策略：拒绝编译并报错**（E2106），提示用户：
  ...
  - 调整 `MAX_MATCH_SET_LEN`（需同步 `matcher.rs` 与 `tproxy.c` 的常量，且受 eBPF
    栈/位图内存限制）。
- **可选降级（阶段 5，默认关闭）**：
  - 域名集索引超限：对域名集做**采样/截断**（如按后缀前缀哈希取子集）并
    告警——不推荐，因会改变匹配语义；
...
### 阶段 0：依赖与骨架
- 新增依赖：HTTP 客户端（`reqwest` 或 `ureq`，含 socks 代理）。
...
### 阶段 3：语法 / matcher 集成
- 引用语法解析与归一化（`source_ip`/`target_ip`/`target_domain` + `set:`；旧别名映射）。
- matcher：`compile_rules()` 接入规则集 → LPM trie / domain_sets；缺失处理
  （E2103）；容量检查（E2106）。
- **验证**：路由编译单测（规则集 → MatchSet/LPM/domain_sets）。
...
- 更新 [`docs/config/config_zh_hans.md`](docs/config/config_zh_hans.md)（配置字段）、
  [`docs/design/routing_zh_hans.md`](docs/design/routing_zh_hans.md)（数据面求值），
  以及英文版。
...
- dae-rs 现有文档：`docs/design/routing_zh_hans.md`、
