#!/bin/bash
# Build the glibc companion libraries required by libbpf-sys for musl cross-compilation:
#   argp-standalone (argp), musl-fts (fts), musl-obstack (obstack)
# elfutils' configure fails on musl when these glibc-only APIs are missing:
#   - "failed to find argp_parse"
#   - "failed to find fts_close"
#
# Usage: ./scripts/build-musl-deps.sh <zig-target> [prefix]
#   e.g. ./scripts/build-musl-deps.sh x86_64-linux-musl
#        ./scripts/build-musl-deps.sh aarch64-linux-musl /tmp/musl-deps
#
# Output prefix directory (default /tmp/dae-rs-musl-deps); afterwards pass
#   CFLAGS_<target>="-I<prefix>/include -L<prefix>/lib"
# to cargo zigbuild so libbpf-sys' elfutils configure can find these libraries.
set -euo pipefail
...
# Equivalent to the config.h generated by its meson build
cat > config.h <<'EOF'
#ifndef _GNU_SOURCE
# undef _GNU_SOURCE
#endif
...
# Keep only static libraries: dynamic .so may be mis-linked when the prefix directory is reused across architectures
